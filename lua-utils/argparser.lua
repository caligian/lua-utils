...
---@overload fun(specs: table): Argparser.Option
...
function Argparser.Positional:initialize(specs)
  ...
  validate {
    name = {types.U('string', 'number'), specs.name},
...
function Argparser.Option:initialize(specs)
  validate {
    opt_metavar = {"string", specs.metavar},
    opt_nargs = {types.U("string", "number"), specs.nargs},
...
function Argparser:parse(args)
  ...
  for i = 1, #index do
    ...
    if not till then
      passed = list.slice(args, from[1] + 1, #args)
      last = from[2]
    else
      passed = list.slice(args, from[1] + 1, till[1] - 1)
    end
  ...
  if nargs == "?" then
    ...
    elseif passed > nargs then
      tail = list.slice(givenargs, 2, -1)
      last.args = { givenargs[1], }
    end
  elseif nargs == "+" then
    if passed == 0 then
      error(
  ...
  elseif types.n(nargs) then
    if nargs > passed then
      error(
        name .. ": expected " .. nargs .. ", got " .. passed
      )
    else
      tail = list.slice(givenargs, nargs + 1, -1)
      last.args = list.slice(givenargs, 1, nargs)
    end
  end
  ...
  if first ~= last then
    if first.index[1] ~= 1 then
      head = list.slice(args, 1, first.index[1] - 1)
    end
  end
...
function Argparser:tostring()
  ...
  if dict.size(self.options) > 0 then
    if pos_set then
      list.append(usage, "")
    end
